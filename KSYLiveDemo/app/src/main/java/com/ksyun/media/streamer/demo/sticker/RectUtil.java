...
public class RectUtil {
    ...
    public static PointF rotatePoint(final PointF srcPoint, final PointF centerPoint, float
            roatetAngle) {
        float x = srcPoint.x;
        float y = srcPoint.y;
        ...
        PointF result = new PointF(newX, newY);
        return result;
     ...
     * @param padding
     */
    public static void rectAddV(final Rect srcRect, final Rect addRect, int padding, int
            minHeight) {
        if (srcRect == null || addRect == null)
            return;
        ...
        bottom += padding + Math.max(addRect.height(), minHeight);
        srcRect.set(left, top, right, bottom);
    }
}
